 ...
 * @Program ：vevor-tools
 * @Author ：Li Hui
 * @Date ：Created in 2020/10/23 10:36
 * @Version ：1.0.1
 */
public abstract class SitemapGenerator <U extends ISitemapUrl, THIS extends SitemapGenerator<U,THIS>>{
    ...
    public THIS addUrl(String url) {
        U sitemapUrl;
        try {
            sitemapUrl = (U) renderer.getUrlClass().getConstructor(String.class).newInstance(url);
    ...
    public THIS addUrl(URL url) {
        U sitemapUrl;
        try {
            sitemapUrl = (U) renderer.getUrlClass().getConstructor(URL.class).newInstance(url);
            return addUrl(sitemapUrl);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    private void writeSiteMap() throws IOException {
        ...
        try {
            if (gzip) {
                ...
                out = new OutputStreamWriter(gzipStream, StandardCharsets.UTF_8.newEncoder());
            } else {
                out = new OutputStreamWriter(new FileOutputStream(outFile), StandardCharsets.UTF_8.newEncoder());
            }
